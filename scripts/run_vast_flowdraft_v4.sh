...
mkdir -p "$HF_HOME" "$HF_DATASETS_CACHE" "$HF_MODULES_CACHE" "$XDG_CACHE_HOME" \
...
OUT_DIR="${OUT_DIR:-/workspace/flowdraft_runs/flowdraft_v4_$(date -u +%Y%m%dT%H%M%SZ)}"
MAX_STEPS="${MAX_STEPS:-300}"
NUM_ANCHOR_BLOCKS="${NUM_ANCHOR_BLOCKS:-64}"
...
log "FlowDraft v4 resource preflight"
"$PYTHON_BIN" scripts/inspect_resources.py --paths / /workspace /dev/shm

log "Training teacher-forced categorical FlowDraft v4"
"$PYTHON_BIN" scripts/train_flowdraft.py \
  --config configs/flowdraft_v4.yaml \
...
log "FlowDraft v4 run complete: $OUT_DIR"
