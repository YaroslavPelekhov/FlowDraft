...
log "Dynamic-support CFM resource preflight"
...
"$PYTHON_BIN" scripts/train_dynamic_support_flow.py \
  ...
  --output-dir "$OUT_DIR" \
  --max-steps "$MAX_STEPS"
CHECKPOINT="$OUT_DIR/best"; [ -f "$CHECKPOINT/dynamic_support_flow.safetensors" ] || CHECKPOINT="$OUT_DIR/last"
log "Strict FP32 greedy losslessness gate with dynamic semantic support"
"$PYTHON_BIN" scripts/benchmark_dynamic_support_flow.py \
...
log "Dynamic-support CFM probe complete: $OUT_DIR"
