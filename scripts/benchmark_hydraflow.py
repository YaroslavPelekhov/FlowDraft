#!/usr/bin/env python
"""Strict greedy benchmark for self-conditioned HydraFlow decoding."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Benchmark one-pass HydraFlow against greedy AR.")
...
def load_head(path: str | Path, device: torch.device, dtype: torch.dtype):
    ...
    head.load_state_dict(load_file(path / "hydraflow_head.safetensors"), strict=True)
...
def main() -> None:
    args = parse_args()
    device, dtype = torch.device("cuda" if torch.cuda.is_available() else "cpu"), dtype_from_string(args.dtype)
    ...
    total_ar_tokens, total_draft_tokens = sum(row["ar_tokens"] for row in rows), sum(row["hydraflow_tokens"] for row in rows)
    total_ar_seconds, total_draft_seconds = sum(row["ar_seconds"] for row in rows), sum(row["hydraflow_seconds"] for row in rows)
    total_target_forwards, total_head_calls = sum(row["target_model_forward_passes"] for row in rows), sum(row["hydraflow_head_calls"] for row in rows)
    accepted = [value for row in rows for value in row["acceptance"]]
    summary = {"method": "HydraFlow self-conditioned latent feature drafter", "num_prompts": len(rows), "checkpoint": str(Path(args.checkpoint).resolve()), "base_checkpoint": str(parent), "dtype": args.dtype, "parity_rate": sum(row["parity_ok"] for row in rows) / len(rows) if rows else 0.0, "aggregate_speedup": ((total_draft_tokens / total_draft_seconds) / (total_ar_tokens / total_ar_seconds)) if total_ar_seconds and total_draft_seconds else 0.0, "aggregate_target_model_tpf": total_draft_tokens / total_target_forwards if total_target_forwards else 0.0, "weighted_acceptance": sum(accepted) / len(accepted) if accepted else 0.0, "p50_acceptance": percentile([float(value) for value in accepted], 0.5), "total_target_model_forward_passes": total_target_forwards, "total_hydraflow_head_calls": total_head_calls}
