...
OUT_DIR="${OUT_DIR:-/workspace/flowdraft_runs/flowdraft_v5_prefix_ecld_2000_r1}"
...
if [ ! -f "$INIT_CHECKPOINT/adapter_config.json" ]; then
  log "Missing CFM initialization checkpoint: $INIT_CHECKPOINT"; exit 2
fi
mkdir -p "$OUT_DIR"; exec > >(tee -a "$OUT_DIR/run.log") 2>&1
log "Long CFM resource preflight"
"$PYTHON_BIN" scripts/inspect_resources.py --paths / /workspace /dev/shm
log "Training verifier-anchored categorical Flow Map with prefix objective"
"$PYTHON_BIN" scripts/train_flowdraft.py \
  --config configs/flowdraft_v5_prefix_ecld_2000.yaml \
  --init-checkpoint "$INIT_CHECKPOINT" \
  --train-manifest "$TRAIN_MANIFEST" \
  --eval-manifest "$EVAL_MANIFEST" \
  --output-dir "$OUT_DIR" \
  --max-steps "$MAX_STEPS"
CHECKPOINT="$OUT_DIR/best"; [ -f "$CHECKPOINT/adapter_config.json" ] || CHECKPOINT="$OUT_DIR/last"
log "Strict eager FP32 greedy losslessness gate"
"$PYTHON_BIN" scripts/benchmark_flowdraft.py \
  ...
  --max-new-tokens 64 --flow-steps 1 --dtype fp32 --attn-implementation eager \
  --parity-margin-threshold 0 --require-parity
if [ -n "$HF_REPO_ID" ]; then
  log "Uploading best/last CFM checkpoints and protocol to Hugging Face"
...
log "Long CFM run complete: $OUT_DIR"
