...
log "SimplexFlow CFM resource preflight"
"$PYTHON_BIN" scripts/inspect_resources.py --paths / /workspace /dev/shm
log "Training local-simplex categorical flow map"
"$PYTHON_BIN" scripts/train_simplex_flow.py --config configs/simplex_flow_probe.yaml --init-checkpoint "$INIT_CHECKPOINT" --train-manifest "$TRAIN_MANIFEST" --eval-manifest "$EVAL_MANIFEST" --output-dir "$OUT_DIR" --max-steps "$MAX_STEPS"
CHECKPOINT="$OUT_DIR/best"; [ -f "$CHECKPOINT/simplex_flow.safetensors" ] || CHECKPOINT="$OUT_DIR/last"
log "Strict FP32 greedy losslessness gate; one Orthrus diffusion Qwen pass per proposal cycle"
"$PYTHON_BIN" scripts/benchmark_simplex_flow.py --checkpoint "$CHECKPOINT" --prompts-jsonl eval_prompts/quick_compare.jsonl --output-jsonl "$OUT_DIR/benchmark_fp32_metrics.jsonl" --summary-json "$OUT_DIR/benchmark_fp32_summary.json" --max-new-tokens 64 --flow-steps 2 --dtype fp32 --attn-implementation eager --require-parity
...
log "SimplexFlow probe complete: $OUT_DIR"
