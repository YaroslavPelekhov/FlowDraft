...
OUT_DIR="${OUT_DIR:-/workspace/flowdraft_runs/hydraflow_probe_300_r1}"
...
log "HydraFlow resource preflight"
"$PYTHON_BIN" scripts/inspect_resources.py --paths / /workspace /dev/shm
log "Training self-conditioned latent feature flow"
"$PYTHON_BIN" scripts/train_hydraflow.py \
  --config configs/hydraflow_probe.yaml \
...
log "Strict FP32 greedy losslessness gate; TPF counts only frozen-Qwen forwards"
...
if [ -n "$HF_REPO_ID" ]; then
  log "Uploading HydraFlow best/last checkpoints to Hugging Face"
...
log "HydraFlow probe complete: $OUT_DIR"
